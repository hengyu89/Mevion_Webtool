...
当前版本：`v2.1.2`
当前分支：`main`  
当前状态：按用户要求将全部修改统一归入 v2.1.2。Patient Counter 只显示具有明确治疗野剂量落盘证据的病人和射野；定位、复位、Setup、计划打开或只选野但未见出束的活动不显示。下方 v2.1.3–v2.1.6 仅保留为开发过程记录，不作为当前发布版本。修改尚未提交 Git；127/127 项自动测试通过。
...
- 根据用户与同事最终确认，列表只保留至少有一个原始Beam>=2明确剂量记录的病人；定位、复位、纯Setup、计划打开或只选野但未见出束的病人整行不显示。
- 「治疗射野」只显示明确剂量落盘的原始Beam>=2，并使用粗体深色；原始Beam1（Setup）及只打开/选择但未落盘的野不显示。治疗人数、治疗Frac/NEW、行内治疗野数和右侧日/总统计均采用相同证据口径，同日同病人同Beam去重。
- 治疗时段改按 Treatment UID（缺失时按剂量路径 SESSION）区分，并仅汇总有治疗野剂量证据的时段，避免同一病人多次独立 Frac1 操作被合并成一大段。
- 三份真实日志本机回放：确认治疗37人、治疗Frac1为1人、9月1日治疗野合计122；60019819707无治疗野剂量记录，因此不显示。最长有效治疗耗时为42.8分钟，不再出现174.3分钟。原始日志未复制入仓库。
- 首页和提取逻辑说明已同步，Patient Counter JS缓存版本更新为20260902-4；127项测试通过，包含定位/复位过滤、选野但未落盘、已治疗病人夹带未治疗选野、同Frac多时段等回归边界。
